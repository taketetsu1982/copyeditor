...
def validate_final(payload, originals=(), *, tool="polish_text", expected_enabled=False, registry=snapshot, format="text"):
    encoded = validate_shape(payload, tool=tool)
    if tool == "lint_text":
    ...
    if not costs or None in costs or len({cost["currency"] for cost in costs}) != 1:
        ...
        # A total rounds once; two independently rounded rows may differ by one micro-unit.
        difference = abs(Fraction(payload["cost"]["amount"]) - sum(Fraction(cost["amount"]) for cost in costs))
        valid(difference <= Fraction(len(costs) - 1, 1000000))
    ...
    if payload["status"] == "error":
        ...
        if enabled:
            ...
            verified = any(item["detection"]["status"] == "eligible" and item["text"] != original.text
                           and not (item["flag"] and item["flag"]["kind"] == "unfixable")
                           for item, original in zip(entries, originals))
            valid(1 + int(verified) <= count <= evaluated + 2 * generated if evaluated else count == 0)
