...
| `protected_terms` | `COPYEDITOR_PROTECTED_TERMS` |
...
**Rewrite with a diagnosis.** Use degree=polish for ordinary proofreading and degree=rewrite only for explicit rewriting. Both return generation 4. Rewrite generates the candidate and a one-line diagnosis together; polish diagnosis is null. For example, after permission checks:
...
HTML uses a whole-document text request, never items or partial recovery. Rewrite generates batches of up to four. Both degrees share at most one retry per item across deterministic and judgment checks. Request errors discard all candidates and diagnoses. HTML input validation remains; HTML structure comparison is retired.
...
**Accounting and acceptance.** Rewrite permits at most 16 generation calls and a 240-second deadline. CountTokens can transmit text before generation, so zero calls does not prove zero transmission. Provider usage includes failures and retries; missing components remain unknown. Estimates and cancellation do not prove billed cost or remote cancellation. See [accounting](contracts/tools.md#current-edit-payloads).
...
Under [CTR-02](rules/common.md#meaning-and-adoption), flags keep local originals even when the response retains a candidate. Apply unflagged bodies only within authorized ranges, or show a diff if application is forbidden. Markdown structure changes remain for review. The Skill adds no semantic comparison, local preservation recheck, related-item cascade or last-moment original comparison.
...
**見立てつき書き直し。** 通常はdegree=polish、明示された書き直しだけdegree=rewriteを使います。どちらも世代4を返します。rewriteは候補と一行の見立てを同時生成し、polishの見立てはnullです。許可確認後の要求例です。
...
HTMLは全文text要求だけを使い、itemsや部分回復は行いません。rewriteは最大4 itemずつ生成します。両degreeとも決定的検査と判定で共有する再生成はitemごとに最大一回です。要求エラーでは全候補・見立てを破棄します。HTML入力の検証は維持し、構造比較は廃止します。
...
**計量と受入。** rewriteは生成最大16回・期限240秒です。CountTokensで生成前に本文を送る場合があり、呼出しゼロは送信ゼロを意味しません。provider別usageは失敗・再生成を含め、欠測は不明のまま扱います。見積もりやcancelは請求額・リモート取消しを保証しません。[計量](contracts/tools.md#current-edit-payloads)を参照してください。
...
[CTR-02](rules/common.md#meaning-and-adoption)に従い、flagがあれば応答に候補が残っていても原文を保持します。flagなしは許可範囲だけへ反映し、反映禁止なら差分を示します。Markdown構造の変化は要確認です。Skillによる意味比較・保持再検査・関連item連鎖・反映直前の原文比較は追加しません。
...
サーバーは送信本文・候補を永続保存しません。stdoutには[CTR-01の監査項目](contracts/tools.md#audit-log)だけ、stderrには起動診断を出します。providerの保持方針やインフラログはこの範囲に含まれないため、別途確認してください。校正に送った本文はVertex AIへ渡り、判定有効時はTypeSafe AIも追加されます。その保持方針と処理地域は同providerの方針に従います。判定値や理由は監査ログに追加せず、provider別の全計量は世代4応答で確認します。
