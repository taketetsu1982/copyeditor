...
cp "$root/scripts/benchmark_provider.py" "$root/scripts/benchmark_assert.py" "$root/scripts/examples_to_promptfoo.py" "$root/scripts/test_fixtures.sh" "$root/scripts/build_test_image.sh" "$work/scripts/"
cp "$root/tests/integration/test_benchmark.py" "$work/tests/integration/"
bash "$root/scripts/build_test_image.sh" fixtures --tag "$name" "$work"
