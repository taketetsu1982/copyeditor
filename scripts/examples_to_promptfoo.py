...
def convert(output, cases, live=False):
    ...
    config = dict(prompts=["{{bad}}"], providers=[dict(id=relative("benchmark_provider.py"), config=dict(mode="live" if live else "fixture"))],
                  tests=[dict(description=c["language"] + "/" + c["id"], vars=c, options=dict(disableVarExpansion=True), **{"assert": [dict(type="python", value=relative("benchmark_assert.py"), config=dict(mode="live" if live else "fixture"))]}) for c in cases])
